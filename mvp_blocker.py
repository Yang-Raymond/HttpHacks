...
class Logger:
    ...
    async def write(self, kind, host, port, decision, detail=""):
        # detail is optional to stay compatible with AppBlocker
...
def _load_apps(path: str, cli_list):
    apps = list(cli_list or [])
...
async def main_async(args):
    ...
    # Start PAC server first so we can print the URL
    pac = start_pac_server(args.pac_port, args.proxy_port, args.socks_port)
    ...
    # App blocker
    patterns = _load_apps(args.apps_file, args.app)
    blocker_task = None
    if patterns:
        blocker = AppBlocker(
            patterns=patterns,
            ...
            scan_interval=args.app_interval,
            logger=logger,
            dry_run=args.app_dry_run
        )
        blocker_task = asyncio.create_task(blocker.run())
        print(f"[APP] Blocking enabled for: {', '.join(patterns)} (mode={args.app_mode})")

    print("\n[INFO] Press Ctrl+C to stop")
    print("[INFO] Blocking is active\n")

    # Run everything concurrently
    tasks = [asyncio.create_task(http.run()), asyncio.create_task(socks.run())]
    if blocker_task: tasks.append(blocker_task)
...
def main():
    global _pac_enabled
    p = argparse.ArgumentParser("MVP Domain Blocker (proxy+PAC+apps, Python)")
    ...
    # App blocker flags
    p.add_argument("--app", action="append", default=[], help="Process name wildcard to block (repeatable)")
    p.add_argument("--apps-file", type=str, default="", help="JSON file with {'apps': ['discord*','steam*']}")
    p.add_argument("--app-mode", choices=["polite","strict"], default="polite", help="Terminate only vs terminate→kill")
    p.add_argument("--app-interval", type=float, default=2.0, help="Process scan interval seconds")
    p.add_argument("--app-grace", type=float, default=2.0, help="Grace seconds before kill() in strict")
    p.add_argument("--app-dry-run", action="store_true", help="Log matches but do not terminate")
